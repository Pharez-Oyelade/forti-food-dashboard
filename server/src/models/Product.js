...
productSchema.virtual('stock_value_at_cost').get(function () {
  return (this.units_on_hand || 0) * (this.unit_cost || 0);
});

productSchema.virtual('sell_through_rate').get(function () {
  const totalReceived = (this.units_on_hand || 0) + (this.units_sold_to_date || 0);
...
productSchema.pre('save', function (next) {
  if (this.isModified('units_on_hand') || this.isModified('expiry_date')) {
    if (this.units_on_hand === 0) {
      this.status = INVENTORY_STATUS.DEPLETED;
    } else if (this.units_on_hand <= this.reorder_point) {
      this.status = INVENTORY_STATUS.SLOW_MOVER; // Or Low Stock
